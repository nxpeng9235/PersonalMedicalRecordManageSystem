...
public class DoctorSeeHRFrame extends JFrame {
	
	private JPanel jpSearch, jpTable, jpButton;
	private JLabel jlb1;
	private JTextField jtf1;
    private JButton jbtSearch, jbtNew, jbtDetail, jbtDelete, jbtQuit;
	private JTable jt1;
	...
	public DoctorSeeHRFrame(String pid) {
		initGUI(pid);
	}
    ...
    private void initGUI(String pid) {
        ...
        jbtNew = new JButton("新建病历信息");
        jbtDetail = new JButton("查看 / 修改病历");
        jbtDelete = new JButton("删除病历信息");
        jbtQuit = new JButton("退出");

        // 向表格中添加体检报告信息
        healthRecords = HealthRecordDAO.getHealthRecordList(pid);
        ...
        jbtSearch.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                if (strDate.isEmpty()) {
                    ...
                    // 重新获取体检报告列表
                    healthRecords = HealthRecordDAO.getHealthRecordListByDate(pid, strDate);
                    // 更新数据
                    fillRows();
                    // 修改表格数据
                    ((DefaultTableModel) jt1.getModel()).fireTableDataChanged();
        ...
        jbtNew.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent event) {
                NewHealthRecordFrame hrf1 = new NewHealthRecordFrame(pid);
            }
        });
        ...
        jbtDetail.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent event) {
                ...
                if (index > -1) {
                    ...
                    HealthRecordFrame hrf1 = new HealthRecordFrame(true, str);
                }
            }
        });
        ...
        jbtDelete.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent event) {
                int choice = JOptionPane.showConfirmDialog(null, "是否确定删除该条记录？", "删除记录", JOptionPane.YES_NO_OPTION);
                if (choice == JOptionPane.YES_OPTION) {
                    int index = jt1.getSelectedRow();
                    if (index > -1) {
                        String str = (String) jt1.getValueAt(index, 0);
                        int result = HealthRecordDAO.delete(str);
                        if (result > 0) {
                            ...
                            JOptionPane.showMessageDialog(null, "记录删除失败，请重试！", "失败", JOptionPane.WARNING_MESSAGE);
                        }
                    }
                }
            }
