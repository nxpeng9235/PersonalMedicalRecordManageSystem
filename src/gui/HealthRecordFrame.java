...
public class HealthRecordFrame extends JFrame {
	...
	private JLabel jlbID, jlbPid, jlbDate, jlbDid, jlbFeature, jlbYX,
		jlbJY, jlbDiagnosis, jlbMeasures, jlbAdvice;
	private JTextField jtfID, jtfPid, jtfDate, jtfDid;
	private JTextArea jtaFeature, jtaYX, jtaJY, jtaDiagnosis,
		jtaMeasures, jtaAdvice;
	private JButton jbtSave, jbtQuit;
	private JScrollPane jsp1, jsp2, jsp3, jsp4, jsp5, jsp6;
	
	public HealthRecordFrame(boolean editable, String id) {
		initGUI(editable, id);
	}
	...
	private void initGUI(boolean editable, String id) {
		...
		HealthRecord hr = HealthRecordDAO.getHealthRecordByID(id);
		jtfID.setText(hr.getID());
		jtfPid.setText(hr.getPID());
		jtfDid.setText(hr.getDID());
		jtfDate.setText(hr.getDate());
		jtaFeature.setText(hr.getFeature());
		jtaYX.setText(hr.getYX());
		jtaJY.setText(hr.getJY());
		jtaDiagnosis.setText(hr.getDiagnosis());
		jtaMeasures.setText(hr.getMeasures());
		jtaAdvice.setText(hr.getAdvice());		
		...
		jtaFeature.setEditable(editable);
		jtaYX.setEditable(editable);
		jtaJY.setEditable(editable);
		jtaDiagnosis.setEditable(editable);
		jtaMeasures.setEditable(editable);
		jtaAdvice.setEditable(editable);
	...
	private void saveHR() {
		...
		hr1.setID(Integer.valueOf(jtfID.getText().trim()));
		...
		int result = HealthRecordDAO.update(hr1);
		if (result > 0) {
			JOptionPane.showMessageDialog(null, "体检报告保存成功！");
			this.dispose();
		} else {
			JOptionPane.showMessageDialog(null, "体检报告保存失败，请重试！", "失败", JOptionPane.WARNING_MESSAGE);
		}
	}	
}
